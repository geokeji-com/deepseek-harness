...
test('rejects non-numeric lifecycle durations instead of propagating NaN', async () => {
  ...
  try {
    ...
    process.env.TEAM_WORKSPACE_PER_USER = 'true'
    const config = loadConfig()
    assert.equal(config.idleMs, 7_200_000)
    assert.equal(config.startTimeoutMs, 45_000)
    assert.equal(config.cookieSecure, false)
    assert.equal(config.pathPolicy.perUserWorkspace, true)
    ...
    assert.throws(() => loadConfig(), /AUTH_COOKIE_SECURE must be "true" or "false"/u)
    process.env.AUTH_COOKIE_SECURE = 'false'
    process.env.TEAM_WORKSPACE_PER_USER = 'sometimes'
    assert.throws(
      () => loadConfig(),
      /TEAM_WORKSPACE_PER_USER must be "true" or "false"/u,
    )
  } finally {
